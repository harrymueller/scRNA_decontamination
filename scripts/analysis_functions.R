...
analyse_recluster <- function(files, samples.combined) {
  ...
  suppressMessages({
    plot_pie_ct(samples.combined, config$method, files$ocra_rel_dir, config$plot_cts)
  })
  ...
  barplots_ct_props(tables[[1]]$tables,paste(files$output, "/plots/barplot_ct_prop_fresh.png",sep=""))
  barplots_ct_props(tables[[2]]$tables,paste(files$output, "/plots/barplot_ct_prop_meoh.png",sep=""))
}
...
plot_pie_ct <- function (samples, method, output, plot_cts) {
  ...
  lapply(samples, function(x) {
    ...
    # paper order = PT, Endothelial, Macrophages, CD_IC, CNT, Other
    
    # plotting
    fig <- plot_ly(df, labels = ~labels, values = ~prop, type = 'pie',textinfo = 'label+percent',sort=F)
...
################################################################################################
contigency_table_ct <- function (all_sample_ids, cell_annotations_path, new_clus_path, output_file) {
...
################################################################################################
get_cont_table <- function (all_sample_ids, cell_annotations_path, new_clus_path, use_new_clus) {
  orig_cts = data.frame()
  # getting all barcodes and cts
  for (id in all_sample_ids) {
    t = get_clusters(cell_annotations_path, id, use_new_clus)
